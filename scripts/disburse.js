...
async function run() {
  ...
  const token = await ethers.getContractAt(
    '/contracts/v4-core/test/ERC20Mintable.sol:ERC20Mintable',
    await yieldSource.depositToken(),
  );
  const decimals = await token.decimals();

  const signers = await ethers.getSigners();

  const addresses = process.env.DISBURSE_ADDRESSES ? process.env.DISBURSE_ADDRESSES.split(',') : '';
  const mintTokenTo = [signers[0].address, ...addresses];

  console.log(chalk.yellow(`Disbursing token ${token.address}...`));

  for (let index = 0; index < mintTokenTo.length; index++) {
    if (mintTokenTo[index].length < 42) continue;
